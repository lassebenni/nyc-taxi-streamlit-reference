...
You are on **`practice-kpi-metrics`**: the live class build for Ch4-Ch5 and a focused solo exercise. `render_kpi_panel` is stubbed; the daily-trend and freshness panels are already provided.

Follow **`EXERCISE.md`** in this branch, then compare against `practice-kpi-metrics-solution`.
...
```bash
git switch practice-kpi-metrics
