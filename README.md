...
### Practice solution · Error handling
...
You are on **`practice-error-handling-solution`**: the reference solution (the failing database-side panel wrapped in `try/except`). Compare it against your own attempt on `practice-error-handling`.
...
```bash
git switch practice-error-handling-solution
