...
### Advanced · Batch inputs with st.form
...
⚙️ **Advanced, beyond the Week 11 chapters (optional).** You are on **`practice-form`**: wrap the filter widgets in `st.form` so the query runs once on submit instead of on every widget change.

Follow **`EXERCISE.md`**, then compare against `practice-form-solution`.
