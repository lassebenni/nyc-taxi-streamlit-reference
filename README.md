...
### Chapter 4 starter · Streamlit Fundamentals
...
You are on **`chapter-4-start`**: the bare starting point for the *Streamlit Fundamentals* chapter (imports, credential loading, and a title only).

Follow the chapter and add each primitive (`st.metric`, `st.columns`, `st.line_chart`) and the cached `run_query` helper directly to `app.py`. The next chapter continues on `chapter-5-start`.
