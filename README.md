...
### Advanced solution · Session state + cached resource
...
⚙️ **Advanced (optional).** You are on **`practice-advanced-state-solution`**: `@st.cache_resource` on the engine and the counter held in `st.session_state`. Compare against your attempt on `practice-advanced-state`.
...
```bash
git switch practice-advanced-state-solution
