...
### Advanced solution · Batch inputs with st.form
...
⚙️ **Advanced (optional).** You are on **`practice-form-solution`**: the filter widgets wrapped in `st.form` with a submit button. Compare against your attempt on `practice-form`.
...
```bash
git switch practice-form-solution
