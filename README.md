...
You are on **`practice-caching-solution`**: the reference solution for the caching exercise (`run_query` wrapped in `@st.cache_data(ttl=300)`). Compare it against your own attempt on `practice-caching`.
...
```bash
git switch practice-caching-solution
