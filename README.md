...
You are on **`practice-error-handling`**: a focused exercise on keeping one broken panel from crashing the whole app. A database-side query fails; you wrap it in `try/except` so the rest of the dashboard still renders, mirroring the Chapter 5 pattern.

Follow **`EXERCISE.md`**, then compare against `practice-error-handling-solution`.
...
```bash
git switch practice-error-handling
